        ...
        <div className="col-span-4 space-y-4 max-h-[calc(100vh-180px)] overflow-y-auto pr-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {/* Sección de Ortoimágenes Propias */}
              {(user?.role === 'administrador' || user?.role === 'coordinador' || (user?.role === 'gestor' && user?.puede_actualizar_gdb)) && (
                <div className="pt-2 border-t mt-2">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs text-slate-500 flex items-center gap-1">
                      <Image className="w-3 h-3" /> Ortoimágenes Propias
                    </label>
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-6 text-xs border-blue-500 text-blue-700 hover:bg-blue-50"
