...
async def export_predios_excel(
    ...
    # Escribir datos R1 (expandiendo múltiples propietarios)
    row_num = 2
    for predio in predios:
        propietarios = predio.get('propietarios', [])
        if not propietarios:
            # Si no hay propietarios en array, usar los campos individuales
            propietarios = [{
                'nombre': predio.get('nombre_propietario', ''),
        ...
        total_registros = len(propietarios)
        
        for idx, prop in enumerate(propietarios, 1):
            row_data = [
                predio.get('departamento', '54'),
                predio.get('municipio', ''),
                predio.get('numero_predio', ''),
                ...
                predio.get('avaluo', 0),
                predio.get('vigencia', ''),
                predio.get('tipo_mutacion', ''),
                predio.get('numero_resolucion', ''),
                predio.get('fecha_resolucion', '')
            ]
            
            for col, value in enumerate(row_data, 1):
                cell = ws_r1.cell(row=row_num, column=col, value=value)
                cell.border = thin_border
            
            row_num += 1
    
    # === HOJA R2 ===
    ws_r2 = wb.create_sheet("REGISTRO_R2")
    ...
    for predio in predios:
        ...
        row_data = [
            ...
            1,  # NUMERO_DE_ORDEN
            1,  # TOTAL_REGISTROS
            r2.get('matricula_inmobiliaria', ''),
            r2.get('zona_fisica_1', 0),
            r2.get('zona_economica_1', 0),
            r2.get('area_terreno_1', 0),
            r2.get('zona_fisica_2', 0),
            r2.get('zona_economica_2', 0),
            r2.get('area_terreno_2', 0),
            r2.get('habitaciones_1', 0),
            r2.get('banos_1', 0),
            r2.get('locales_1', 0),
            r2.get('pisos_1', 0),
            r2.get('tipificacion_1', 0),
            r2.get('uso_1', 0),
            r2.get('puntaje_1', 0),
            r2.get('area_construida_1', 0),
            r2.get('habitaciones_2', 0),
            r2.get('banos_2', 0),
            r2.get('locales_2', 0),
            r2.get('pisos_2', 0),
            r2.get('tipificacion_2', 0),
            r2.get('uso_2', 0),
            r2.get('puntaje_2', 0),
            r2.get('area_construida_2', 0),
            predio.get('vigencia', '')
        ]
        
        for col, value in enumerate(row_data, 1):
            cell = ws_r2.cell(row=row_num, column=col, value=value)
            cell.border = thin_border
