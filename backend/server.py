...
def generate_certificado_catastral(predio: dict, numero_certificado: str, firmante: dict) -> bytes:
    ...
    # Título principal
    story.append(Paragraph("<b>CERTIFICADO CATASTRAL</b>", title_style))
    
    # Validez legal
    story.append(Paragraph(
        "ESTE CERTIFICADO TIENE VALIDEZ DE ACUERDO CON LA LEY 527 DE 1999 (AGOSTO 18) "
        "Directiva Presidencial No. 02 del 2000, Ley 962 de 2005 (Anti trámites), Artículo 6, Parágrafo 3.",
        small_style
    ))
    story.append(Spacer(1, 12))
    ...
    # Entidad emisora
    story.append(Paragraph(
        "<b>LA ASOCIACIÓN DE MUNICIPIOS DEL CATATUMBO PROVINCIA DE OCAÑA Y SUR DEL CESAR – ASOMUNICIPIOS</b>",
        ParagraphStyle('Entity', parent=styles['Normal'], fontSize=10, alignment=TA_CENTER)
    ))
    story.append(Paragraph(
        "certifica que el siguiente predio se encuentra inscrito en la base de datos catastral con la siguiente información:",
        ParagraphStyle('Cert', parent=styles['Normal'], fontSize=9, alignment=TA_CENTER, spaceAfter=12)
    ))
    
    # INFORMACIÓN CATASTRAL DEL PREDIO
    story.append(Paragraph("<b>INFORMACIÓN CATASTRAL DEL PREDIO</b>", section_style))
    story.append(Paragraph(f"Predio No.: {predio.get('numero_predio', '01')}", normal_style))
    
    # INFORMACIÓN JURÍDICA
    story.append(Paragraph("<b>INFORMACIÓN JURÍDICA</b>", section_style))
    
    propietarios = predio.get('propietarios', [])
    if propietarios:
        for i, prop in enumerate(propietarios, 1):
            story.append(Paragraph(f"<b>Propietario {i}:</b>", normal_style))
            story.append(Paragraph(f"Nombre: {prop.get('nombre_propietario', '')}", normal_style))
            story.append(Paragraph(f"Tipo de documento: {prop.get('tipo_documento', '')}", normal_style))
            story.append(Paragraph(f"Número de documento: {prop.get('numero_documento', '')}", normal_style))
    else:
        story.append(Paragraph(f"Nombre del propietario: {predio.get('nombre_propietario', 'N/A')}", normal_style))
    ...
    story.append(Paragraph(f"Matrícula Inmobiliaria: {matricula}", normal_style))
    
    # INFORMACIÓN FÍSICA
    story.append(Paragraph("<b>INFORMACIÓN FÍSICA</b>", section_style))
    story.append(Paragraph(f"Departamento: {predio.get('departamento', '54 - NORTE DE SANTANDER')}", normal_style))
    story.append(Paragraph(f"Municipio: {predio.get('municipio', '')}", normal_style))
    story.append(Paragraph(f"Número predial: {predio.get('codigo_predial_nacional', '')}", normal_style))
    story.append(Paragraph(f"Número predial anterior: {predio.get('codigo_homologado', '')}", normal_style))
    story.append(Paragraph(f"Dirección: {predio.get('direccion', '')}", normal_style))
    ...
    story.append(Paragraph(f"Área terreno: {area_str}", normal_style))
    story.append(Paragraph(f"Área construida: {predio.get('area_construida', 0)} m²", normal_style))
    
    # INFORMACIÓN ECONÓMICA
    story.append(Paragraph("<b>INFORMACIÓN ECONÓMICA</b>", section_style))
    avaluo = predio.get('avaluo', 0)
    avaluo_str = f"$ {avaluo:,.0f}".replace(',', '.')
    story.append(Paragraph(f"Avalúo catastral: {avaluo_str}", normal_style))
    story.append(Spacer(1, 20))
    
    # Fecha de expedición
    story.append(Paragraph(
        f"El presente certificado se expide a favor del interesado el {fecha_str}.",
        normal_style
    ))
    ...
    firma_data = [
        ['_' * 40],
        [f"<b>{firmante.get('full_name', '')}</b>"],
        [firmante.get('cargo', 'SUBDIRECTOR(A) FINANCIERO(A) Y ADMINISTRATIVO(A)')]
    ]
    firma_table = Table(firma_data, colWidths=[10*cm])
    ...
    # NOTAS LEGALES
    story.append(Paragraph("<b>NOTA:</b>", small_style))
    story.append(Paragraph(
...
async def generar_certificado_catastral(predio_id: str, current_user: dict = Depends(get_current_user)):
